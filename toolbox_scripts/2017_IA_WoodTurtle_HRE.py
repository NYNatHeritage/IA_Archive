...
out_buff = "out_buff"
hydro = "F:/_Schmid/_project/Important_Areas/GIS_Data/IA_Process.gdb/HRE_bat"
...
print("Buffer the EO 10, 300, and 500m for various steps...")
arcpy.Buffer_analysis(in_EOs, "in_put10", 10, "FULL", "ROUND", "ALL")
arcpy.Buffer_analysis(in_EOs, "in_put300", 300, "FULL", "ROUND", "ALL")
arcpy.Buffer_analysis(in_EOs, "in_put500", 500, "FULL", "ROUND", "ALL")
arcpy.MultipartToSinglepart_management("in_put500","in_put500sp")

# Process: Use the Identity function
print("Use the Identity function to locate max distance 500m streams...")
...
print("Select out the contiguous streams within 300m of the points...")    
...
arcpy.Dissolve_management("Dissolve1", "Dissolve12", "IA_MODEL_R", "", "SINGLE_PART")
#Since the SINGLE_PART function in previous step is mysteriously not working:
arcpy.MultipartToSinglepart_management("Dissolve12","Dissolve12sp")


# Select out only those polys contiguous with the EO
arcpy.MakeFeatureLayer_management("Dissolve12sp", "LAYER_Dissolve12sp")
arcpy.SelectLayerByLocation_management("LAYER_Dissolve12sp", "INTERSECT", "in_put10", "", "NEW_SELECTION")
...
arcpy.CopyFeatures_management ("LAYER_Dissolve12sp", FinalFC)
arcpy.CopyFeatures_management ("LAYER_Dissolve12sp", FinalFC_gdb)

print("IA model done: Wood Turtle.")
