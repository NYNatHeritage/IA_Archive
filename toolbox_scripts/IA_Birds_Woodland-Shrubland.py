...
arcpy.AddMessage("Woodland/Shrubland bird EOs >= 61ha differentiated.")
...
for row in rows: 
    ...
    arcpy.AddMessage("WSBIRD_ID = " + rws_ID)
...
#    ElimDist = 6070
    ...
    while checkOne > 0:
        ...
        if SelectCount == 0:
            ...
            arcpy.AddMessage("Feature " + repr(row.WSBIRD_ID) + " still not big enough.")
        else:
            checkOne = 0
            arcpy.Append_management("WS_EO_Diss", "Append_in_wsBirds", "NO_TEST")
            arcpy.AddMessage("Feature " + repr(row.WSBIRD_ID) + " is big enough now, and appended.")
        ...
        if countDist >= TotalDist:
            checkOne = 0
            arcpy.Append_management("WS_EO_Diss", "Append_in_wsBirds", "NO_TEST")
            arcpy.AddMessage("Feature " + repr(row.WSBIRD_ID) + " has reached 0.5km so now it is appended.")
            arcpy.AddMessage("checkOne = " + repr(checkOne) + " buffer attempts.")
...
arcpy.AddMessage("IA model done: Woodland/Shrubland Birds.")
