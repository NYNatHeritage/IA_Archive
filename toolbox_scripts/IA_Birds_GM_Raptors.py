...
arcpy.SelectLayerByAttribute_management("LAYER_NWI_select", "NEW_SELECTION", "\"SYSTEM\" = 'E' OR \"SYSTEM\" = 'P'")
arcpy.SelectLayerByAttribute_management("LAYER_NWI_select", "SUBSET_SELECTION", "\"CLASS1\" <> 'FO'")
arcpy.SelectLayerByAttribute_management("LAYER_NWI_select", "REMOVE_FROM_SELECTION", "\"NWI_CODE\" = 'E1UBL'")
...
arcpy.Union_analysis("CCAP_select #; C:/_Schmid/_project/Important_Areas/GIS_Data/IA_Process.gdb/DECwetlands_DOT_polys #; NWI_wet_polys #", "GrassMarshUnion", "ONLY_FID", "", "NO_GAPS")
arcpy.Dissolve_management("GrassMarshUnion", "GrassMarshDissolve", "", "", "SINGLE_PART")
arcpy.AddMessage("Grasslands and Marsh unioned and dissolved.")
...
arcpy.AddMessage("Raptor EOs and habitat, unioned and dissolved.")
...
for row in rows: 
    ...
    while checkOne > 0:
        ...
        if SelectCount == 0:
            ...
            arcpy.AddMessage("Feature " + repr(row.gm_ID) + " still not big enough.")
        else:
            checkOne = 0
            arcpy.Append_management("gmDissolve2", "Append_in_gmBirds", "NO_TEST")
            arcpy.AddMessage("Feature " + repr(row.gm_ID) + " is big enough now, and appended.")
        ...
        if countDist >= TotalDist:
            checkOne = 0
            arcpy.Append_management("gmDissolve2", "Append_in_gmBirds", "NO_TEST")
            arcpy.AddMessage("Feature " + repr(row.gm_ID) + " has reached 0.5km so now it is appended.")
            arcpy.AddMessage("checkOne = " + repr(checkOne) + " buffer attempts.")
...
arcpy.AddMessage("IA model done: Grassland/Marsh Raptors.")
